...
> **これまで:** 手作業で変更を作り・レビューできるようになりました。**このデモ:** そのレビューを CI で自動化し、**template-typescript-react** へのすべてのプルリクエストに Copilot のチェックを通します — リポジトリ既存の `test.yaml`／`e2e-test.yaml` ワークフローのすぐ隣に。
...
- GitHub Actions ワークフローを追加できる、template-typescript-react の自分のフォーク。
...
同じ考え方を CI に組み込みます。この例は、すべてのプルリクエストで自動レビューを実行し、結果をコメントとして投稿します。**読み取り中心** で、Copilot は `git` の実行とレポートファイルの書き込みはできますが、push や破壊的コマンドは禁止されています。アプリ既存の `test.yaml`（`make ci-test` を実行）や `e2e-test.yaml` ワークフローと並んで配置されます。
