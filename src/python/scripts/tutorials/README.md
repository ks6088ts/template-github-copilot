...
Each script runs **standalone** with `uv run python <script>.py` — no dependency
on the `template_github_copilot` package.
...
All required packages (`github-copilot-sdk`, `pydantic`, `azure-identity`, …)
are declared in [`pyproject.toml`](../../pyproject.toml) and are installed by
`uv sync`:
...
uv run python scripts/tutorials/05_audit_hooks.py --prompt "Explain Python decorators"
